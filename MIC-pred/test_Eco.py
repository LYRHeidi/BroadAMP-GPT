...
config = OrderedDict(
    ...
    weightfile = './weights/Eco_weight/best_checkpoint.pth'
)
...
def get_loader(test=False):
    ...
    dataset = TestDataset(data, chars, 132)
...
def main():
    ...
    pd.DataFrame(predictions).to_csv(config['output_dir']+'/Eco_test_result.csv', sep="\t", header=False, index=False)
