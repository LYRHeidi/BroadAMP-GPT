...
class Linformer_LINE(nn.Module):
    """
    ...
    def __init__(self, input_size, channels, dim_k, dim_ff=256, dim_d=None, dropout_ff=0.15, nhead=4, depth=1, dropout=0.1, activation="gelu", checkpoint_level="C0", parameter_sharing="layerwise", k_reduce_by_layer=0, full_attention=False, include_ff=True, w_o_intermediate_dim=None, decoder_mode=False, causal=False, method="learnable", ff_intermediate=None):
        super(Linformer_LINE, self).__init__()
        ...
        self.seq = layers
        self.to_logits = nn.Linear(input_size*channels, 1)
...
class LinformerLM(nn.Module):
    ...
    def __init__(self, num_tokens, input_size, channels,
        ...
        # self.to_token_emb = nn.Linear(num_tokens, emb_dim)  # for one-hot embedding
        ...
        self.to_logits = identity if return_emb else nn.Linear(input_size*emb_dim, 2)
        # self.to_logits = identity if return_emb else F.log_softmax(input_size * emb_dim, 5)
        self.dropout_tokens = nn.Dropout(dropout_tokens)
        # self.p_value = self.load_p_value()

    def load_p_value(self):
        import math
        import numpy as np

        p_data_file = '../data/traintest_gaoshidai/gaoshidai_danbai_pvalue.txt'
        p_value = []
        with open(p_data_file, 'r') as f:
            reader = csv.reader(f, delimiter='\t')
            header = next(reader)
            for row in reader:
                # p_value.append(-math.log10(float(row[5])))
                if np.isnan(float(row[5])):
                    value = 1.0
                else:
                    value = -math.log10(float(row[5]))
                # print(-math.log10(row[5]))
                # print(value)
                p_value.append(value)

        p_value = torch.from_numpy(np.array(p_value)).float().to('cuda')
    ...
    def forward(self, tensor, **kwargs):
        ...
        # bt, n, _ = tensor.shape
        bt, n = tensor.shape
        tensor = self.to_token_emb(tensor.int())
        # tensor = self.to_token_emb(tensor)

        # val = self.p_value.repeat(bt).reshape(bt, -1).unsqueeze(dim=2)
        # tensor = torch.mul(tensor, val)

        # for b in range(0, bt):
        #     for row in range(0, n):
        #         tensor[b][row] = tensor[b][row] * val[b][row]
