...
def gai_loss_md(pred, target, gmm, noise_var):
    ...
    # gmm = gmm.cuda()
    balancing_term = MVN(gmm['means'].cuda(), gmm['variances'].cuda()+noise_var*I).log_prob(pred.unsqueeze(1)) + gmm['weights'].cuda().log()
    balancing_term = torch.logsumexp(balancing_term, dim=0)
...
class BMCLossMD(_Loss):
    ...
    def forward(self, pred, target):
        noise_var = (self.noise_sigma ** 2).cuda()
...
def bmc_loss_md(pred, target, noise_var):
    ...
    # loss = F.cross_entropy(torch.arange(pred.shape[0]), logits)
    # print(torch.arange(pred.shape[0]))
    loss = F.cross_entropy(logits, torch.arange(pred.shape[0]).cuda())
    # loss = F.cross_entropy(logits, torch.arange(2))
    loss = loss * (2 * noise_var).detach()
    return loss
